...
LOG=/home/zzhang18/nchen3/sfiii/train_async20M.log
PAT='[p]po_sfiii.py --cfgFile config/config_async.yaml'
...
while true; do
  ...
  if ! pgrep -f "$PAT" >/dev/null; then
    if [ "$s" -ge 19900000 ]; then echo "COMPLETE: 20M run finished at step $s"; else echo "DIED: process gone at step $s (see train_async20M.log)"; fi
    break
  fi
