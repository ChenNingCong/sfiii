...
LOG=/home/zzhang18/nchen3/sfiii/train_curriculum.log
PAT='[p]po_sfiii.py --cfgFile config/config_curriculum.yaml'
...
while true; do
  ...
  if ! pgrep -f "$PAT" >/dev/null; then
    if [ "$s" -ge 19900000 ]; then echo "COMPLETE: 20M run finished at step $s"; else echo "DIED: process gone at step $s (see train_curriculum.log)"; fi
    break
  fi
